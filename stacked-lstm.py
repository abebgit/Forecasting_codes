...
for i in range(n_iter):
    ...
    # Stacked-LSTM(
    model = Sequential()
    model.add(LSTM(combo['lstm_units'], return_sequences=True,
                   input_shape=(X_train_main.shape[1], X_train_main.shape[2])))
...
def estimate_complexity(model, X_shape, batch_size, epochs, best_params):
    ...
    return {
        'Big O Training': f"O(epochs * n_samples * LSTM_units^2)",
        ...
        'Space complexity': f"O(LSTM_units^2)"
    }
...
next_day_data = pd.read_excel('Input_data_to_be_predicted.xlsx')
for i in range(1, look_back + 1):
    next_day_data[f'power_lag{i}'] = next_day_data['power'].shift(i)
next_day_data.fillna(method='bfill', inplace=True)
...
# Metrics for the next day prediction
...
print('\nBest Model Next-Day Forecast Performance-Random-LSTM:')
...
model_info = pd.DataFrame({
    ...
    'Epochs Ran': [epochs_ran],
     'Memory before training':  [memory_before],
     'Memory after training':   [memory_after],
     'Actual memory increase':  [memory_increase_mb],
     'Peak memory during training': [peak_memory_mb]
})
...
with pd.ExcelWriter('LSTM_randomsearch_best_model.xlsx') as writer:
...
print("\nAll metrics + forecasts + Big O exported to 'LSTM_randomsearch_best_model.xlsx'")
...
plt.title('Next Day Forecast vs Actual (Stacked-LSTM)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.grid(True, linestyle='--', linewidth=1.5, alpha=0.7)
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("LSTM_forecast_plot-Random-search.pdf")
plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig("LSTM_training_loss.pdf")
plt.show()

# SHAP analysis

print("Starting SHAP analysis (train-only, original features)...")
try:
    # Build a tiling wrapper so the explainer can receive timesteps=1 inputs (template style),
    #    while the trained LSTM still sees the full look_back internally.
    shap_input = tf.keras.Input(shape=(1, X_train_main.shape[2]), name="shap_input_1step")
    tiled = tf.keras.layers.Lambda(lambda z: tf.repeat(z, repeats=look_back, axis=1),
                                   name="tile_to_lookback")(shap_input)
    shap_model = tf.keras.Model(inputs=shap_input, outputs=best_model(tiled), name="best_model_shap_wrapper")

    # Use the LAST timestep features from TRAIN ONLY as "current" predictor vectors,
    #    then add a singleton time dim => (N_train, 1, F).
    X_train_1step = X_train_main[:, -1:, :]  # train-only pool (S, 1, F)

    # Train-only background & sample selection (3D arrays with timesteps=1)
    background_size = min(50, X_train_1step.shape[0])
    background_indices = np.random.choice(X_train_1step.shape[0], background_size, replace=False)
    background_data = X_train_1step[background_indices]  # shape (B, 1, F)

    test_sample_size = min(50, X_train_1step.shape[0])   # <-- draw samples from TRAIN ONLY
    test_indices = np.random.choice(X_train_1step.shape[0], test_sample_size, replace=False)
    X_train_sample = X_train_1step[test_indices]  # shape (S, 1, F)

    # Flattened versions for plotting (timesteps=1 so flattening is safe)
    ...
    # 5) Try DeepExplainer first
    try:
        # DeepExplainer on wrapper so it accepts (S,1,F) and forwards (S,look_back,F) to the trained model
        explainer = shap.DeepExplainer(shap_model, background_data)
        shap_out = explainer.shap_values(X_train_sample)  # could be array or list
        shap_values = shap_out[0] if isinstance(shap_out, list) else shap_out
        expected_value = explainer.expected_value
        if isinstance(expected_value, list):
            expected_value = expected_value[0]
        print("Used DeepExplainer for SHAP (train-only).")
        # DeepExplainer returns (S, 1, F); flatten to (S, F) for plotting/tables
        shap_values = shap_values.reshape(shap_values.shape[0], -1)
    except Exception as de:
        print(f"DeepExplainer failed: {de}. Falling back to KernelExplainer (slower).")

        # KernelExplainer expects 2D arrays.
        def predict_fn_flat(data_2d):
            # data_2d shape: (n_samples, n_features) since timesteps = 1
            data_3d = data_2d.reshape((data_2d.shape[0], 1, X_train_main.shape[2]))
            preds = shap_model.predict(data_3d, verbose=0).flatten()
            return preds

        explainer = shap.KernelExplainer(predict_fn_flat, background_flat)
        shap_out = explainer.shap_values(X_train_flat, nsamples=300)  # increase for more precision
        shap_values = np.array(shap_out)
        expected_value = explainer.expected_value
        print("Used KernelExplainer for SHAP (train-only fallback).")

    # Ensure shap_values is numpy array with shape (n_samples, n_features)
    ...
    shap.summary_plot(shap_values, X_train_flat, feature_names=feature_columns, plot_type='bar', show=False)
    plt.title('Feature Importance (SHAP) ', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('Shap_-importance.png')
    plt.show()

    plt.figure(figsize=(10, 6))
    shap.summary_plot(shap_values, X_train_flat, feature_names=feature_columns, show=False)
    plt.title('SHAP Summary', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('shap_detail.png')
    plt.show()

    # 8) Local explanation: force/waterfall plot for first training sample
    if isinstance(expected_value, np.ndarray):
        ev = float(expected_value) if expected_value.size == 1 else float(expected_value.ravel()[0])
    else:
        ev = float(expected_value)

    try:
        fp = shap.force_plot(ev, shap_values[0], X_train_flat[0], feature_names=feature_columns, matplotlib=False)
        shap.save_html("shap_plot_sample0.html", fp)
        print("Saved interactive force plot to shap_plot_sample0.html")
    except Exception as fp_e:
        print(f"Interactive force plot failed ({fp_e}), trying matplotlib fallback.")
        try:
            shap.force_plot(ev, shap_values[0], X_train_flat[0], feature_names=feature_columns, matplotlib=True)
            plt.tight_layout()
            plt.savefig('shap_plotS.png')
            plt.show()
            print("Saved matplotlib force plot to shap_plot_sample0.png")
        except Exception as plt_e:
            print(f"Matplotlib force plot also failed: {plt_e}")

    # 9) Feature importance table (mean absolute SHAP) — TRAIN ONLY
    mean_abs_shap = np.mean(np.abs(shap_values), axis=0)
    feature_importance_train = (
        pd.DataFrame({'Feature': feature_columns, 'Importance': mean_abs_shap})
          .sort_values('Importance', ascending=False)
          .reset_index(drop=True)
    )
    print("\n[TRAIN-ONLY] Feature Importance based on SHAP values:")
    print(feature_importance_train)

    # (Optional) Save the table for your paper
    feature_importance_train.to_csv("shap_trainonly_feature_importance.csv", index=False)

except Exception as e:
    print(f"Error in SHAP calculation (train-only): {e}")
    print("Continuing without SHAP visualization...")

print("Train-only SHAP visualizations have been saved.")
