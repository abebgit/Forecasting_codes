...
feature_columns = ['speed', 'speed change', 'temp','pitch'] + [f'power_lag{i}' for i in range(1, look_back + 1)]
target_column = 'power'
...
# Validation split (80/20 chronological) on the *scaled* data (before wavelet)
...
# Apply best wavelet to all data
...
def measure_peak_memory_during_training(train_function):

    process = psutil.Process(os.getpid())
    memory_log = []
    running = True

    # Background memory sampling thread
    def monitor():
        while running:
    ...
    peak_memory = max(memory_log) / (1024 ** 2)
    ...
    return {
        ...
        "memory_increase": peak_memory - memory_before
    }
...
complexity = estimate_complexity(best_model, X_train_full_seq.shape, best_params['batch_size'], best_params)

print("\nBig O Complexity Estimates:")
for k, v in complexity.items():
    print(f"{k}: {v}")

# Evaluation on test dateset 
...
next_day_data = pd.read_excel('Input-to-be-predicted.xlsx')
for i in range(1, look_back + 1):
...
model_info = pd.DataFrame({
    'Model': ['Wavelet-LSTM (per-sample DWT)'],
    'Wavelet': [best_params['wavelet']],
...
timing_summary = pd.DataFrame({
    ...
    'Value': [inference_end - inference_start, next_day_prediction_time, total_time]
})
...
with pd.ExcelWriter('W-LSTM-Random-search_best_model.xlsx') as writer:
...
print("\n All metrics + forecasts + Big O exported to 'W-LSTM-randomsearch_best_model.xlsx'")
...
plt.title('Next Day Forecast vs Actual (Wavelet-LSTM-Random-Search)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.grid(True, linestyle='--', linewidth=1.5, alpha=0.7)
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("WaveletLSTM_forecast_plot-Random.pdf")
plt.show()
...
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.title('Training and Validation Loss - Wavelet-LSTM-Random-Search')
plt.legend()
plt.grid(True)
plt.savefig("WaveletLSTM_training_loss-Random-Search.pdf")
plt.show()

# SHAP analysis
print("Starting SHAP analysis...")
try:
    # Generate feature names from the captured approximations and details_list
    ...
    plt.title('Feature Importance (SHAP) - Wavelet-LSTM-Random-Search', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('W-LSTM-Random-shap_feature_importance.pdf')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('Shap_detailed_plot-W-LSTM-Random.pdf')
    plt.show()
